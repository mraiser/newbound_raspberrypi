...
public class MotionDefault implements Motion
{
	...
	{
		SIZEX = Integer.parseInt(p.getProperty("motion-width", "64"));
		SIZEY = Integer.parseInt(p.getProperty("motion-height", "48"));
		MAX = (int)(Double.parseDouble(p.getProperty("motion-tolerance", "0.05")) * 256);
		FACTOR = Double.parseDouble(p.getProperty("noise-factor", "2"));
		...
		MAPFRAMES = 10;  // Fixme - make configurable
	...
	{
		jo.put("motion-tolerance", MAX);
		jo.put("noise-factor", FACTOR);
		jo.put("motion-width", SIZEX);
		jo.put("motion-height", SIZEY);
		return jo;
	}
